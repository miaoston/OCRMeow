...
import { getHistory, clearHistory, saveHistory, getAsset, saveAsset } from "../utils/db";
...
async function checkBundledModelsExist(): Promise<boolean> {
...
function setupSettingsListeners() {
  ...
  const importBtn = document.getElementById("btn-import-models");
...
async function checkModelStatus() {
  ...
  const updateStatus = (el: HTMLElement | null, ready: boolean, bundled: boolean) => {
    ...
    if (ready) {
      if (bundled) {
        el.textContent = isZh ? "已就绪 (内置 BUNDLED)" : "READY (BUNDLED)";
  ...
  if (storagePath) {
    if (hasDet && hasRec) {
      if (isBundled) {
        storagePath.textContent = isZh ? "应用包内置 (Extension Bundle)" : "EXTENSION_BUNDLE";
...
async function downloadModelsAndRunOCR(base64Image: string, sourceName: string) {
  const isZh = getLang() === "zh";
  const btn = document.getElementById("btn-download-models") as HTMLButtonElement;
  const progressDiv = document.getElementById("model-download-progress");
  const progressText = document.getElementById("model-progress-text");
